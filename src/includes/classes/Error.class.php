...
/**
* Error Class - Handles all errors given by ReflexCMS and decides what to do with them based on config settings.
*/
class Error
{
	...
	/**
	 * Error class construction. Registers self as error handler for the entire session and checks config.
	...
	public function sendTechnicalEmail($errno, $errstr, $errfile=false, $errline=false)
	{
		...
		{
			$message = "Your ReflexCMS Installation has generated an unexpected error:
...
Error: $errstr";
			...
			$send = @mail($this->technicalEmail, "Error on your website powered by ReflexCMS", $message, "From: NoReply@ReflexCMS-Errors");
		}
	}
}
